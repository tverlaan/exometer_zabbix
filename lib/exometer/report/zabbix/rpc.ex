defmodule Exometer.Report.Zabbix.Rpc do
  ...
  def handle_call({:ensure_metric, metric, dp, delay}, _from, s) do
    with {:ok, tmpl_id} <- do_template_id(metric, s),
	 {:ok, app_id} <- do_application_id(metric, tmpl_id, s),
	 {:ok, item_id} <- do_item_id(metric, dp, delay, tmpl_id, app_id, s) do
      {:reply, {:ok, item_id}, s}
    else
      {:error, reason} ->
  ...
  defp do_template_id(metric, s) do
    name = Utils.template(metric)
    params = %{ "search" => %{ "name" => name } }
    case rpc_call("template.get", params, s) do
      {:ok, []} ->
	# TODO: groupid as parameter / app env ?
	params = %{ "host" => name,
		    "groups" => %{ "groupid" => 1 },
  ...
  defp do_application_id(metric, tmpl_id, s) do
    name = Utils.application(metric)
    params = %{ "search" => %{ "name" => name } }
  ...
  defp do_item_id(metric, dp, delay, tmpl_id, app_id, s) do
    key = Utils.key(metric, dp)
    params = %{ "search" => %{ "name" => key }, "applicationids" => [app_id] }
